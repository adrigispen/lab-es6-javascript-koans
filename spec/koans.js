  ...
  describe('takes a position from where to start searching', function() {
    ...
    describe('invalid positions get converted to 0', function() {
      it('e.g. `undefined`', function() {
        const findAtPosition = (pos) => 'xyz'.includes('z', pos = 0); 
        expect(findAtPosition(void 0)).toBe(true);
      });
      it('negative numbers', function() {
        const findAtPosition = (pos) => 'xyz'.includes('x', pos < 0 ? 0 : pos); 
        expect(findAtPosition(-2)).toBe(true);
      });
      it('NaN', function() {
        const findAtPosition = (pos) => 'xyz'.includes('xyz', !Number(pos) ? 0 : pos); 
...
describe('destructuring arrays makes shorter code. ', () => {
  ...
  it('swap two variables, in one operation', () => {
    let [x, y] = ['ax', 'why'];
    [y, x] = [x, y]; // this works, other way around works too
...
describe('class creation', () => {
  ...
  it('multiple methods need no commas (opposed to object notation)', function() {
    class User {
      ...
      isLazy()     { return this.everWroteATest == false; }
    }
